...
| [`../../namespaces/router/SPEC.md`](namespaces/router/SPEC.md) | The router chapter of this specification | Chapter 2 |
...
| [ANS-104](https://github.com/ArweaveTeam/arweave-standards/blob/master/ans/ANS-104.md) | The bundled data-item id is SHA-256 of its signature. `headerMatchesId` checks this relationship; it does not verify the signature over header fields. No bundle parser is implemented. SPEC §3 and §9; `src/ar.js`. | Chapter 4 |
...
| [did:jwk Method](https://github.com/quartzjer/did-jwk/blob/main/spec.md) | did:jwk method specification | Chapter 7 |
| [did:key Method](https://w3c-ccg.github.io/did-key-spec/) | did:key method specification | Chapter 7 |
| [did:pkh Method](https://github.com/w3c-ccg/did-pkh/blob/main/did-pkh-method-draft.md) | did:pkh method draft | Chapter 7 |
...
| [Electron protocol registration](https://www.electronjs.org/docs/latest/api/protocol#protocolregisterschemesasprivilegedcustomschemes) | Custom scheme privileges | Chapter 11 |
...
| [ENSIP-5](https://docs.ens.domains/ensip/5) | Seven text keys displayed on the no-website page, with escaping, a 512-character cap and HTTPS-only URL links. SPEC §5.6a; EN-1; `src/ens-protocol.js` (`textRecords`, `textRecordsHtml`). | Chapter 5 |
...
| [Experimental Part A](namespaces/experimental/SPEC.md#part-a--hip-5-_op-on-chain-resolution) | HIP-5 `_op` on-chain resolution | Chapter 4 |
...
| [Handshake §6](namespaces/handshake/SPEC.md#6-the-resolution-algorithm) | Handshake resolution | Chapter 4 |
| [Handshake §10](namespaces/handshake/SPEC.md#10-content-pointers) | Content pointers | Chapter 4 |
| [Handshake deviations](namespaces/handshake/DEVIATIONS.md), [Handshake §9.1](namespaces/handshake/SPEC.md#91-doh-fallback) | Shared cache and DoH limits | Chapter 4 |
...
| [`multiformats`](https://github.com/multiformats/js-multiformats) | Multiformat encodings | Chapter 7 |
...
| [RFC 9848](https://www.rfc-editor.org/rfc/rfc9848) | Bootstrapping TLS Encrypted ClientHello with DNS Service Bindings | Chapter 1, Chapter 2 |
| [RFC 9849](https://www.rfc-editor.org/rfc/rfc9849) | TLS Encrypted Client Hello | Chapter 1, Chapter 2 |
| [Router §6](namespaces/router/SPEC.md#6-classification-of-bare-input), [shared trust states](SPEC.md#4-trust-states) | Namespace selection and trust states | Chapter 4 |
| [Routing](namespaces/router/SPEC.md), [Handshake resolution](namespaces/handshake/SPEC.md#6-the-resolution-algorithm), [Handshake content pointers](namespaces/handshake/SPEC.md#10-content-pointers), [shared trust states](SPEC.md#4-trust-states) | Shared resolution rules | Chapter 3 |
...
See [DEVIATIONS.md](namespaces/router/DEVIATIONS.md) for limitations and
...
See [DEVIATIONS.md](namespaces/handshake/DEVIATIONS.md) for limitations and
[REVIEW.md](REVIEW.md) for unresolved claims.
...
See [DEVIATIONS.md](namespaces/icann/DEVIATIONS.md) for limitations and
[REVIEW.md](REVIEW.md) for unresolved claims.
...
| [`../../SPEC.md` §4.2](SPEC.md) | The Fast / Private switch | Shared Fast/Private policy and trust model. `src/delivery-mode.js` and its tests define policy; browser composition applies it. |
| [`../../namespaces/router/SPEC.md`](namespaces/router/SPEC.md) | The router chapter of this specification | Routing requirements L1/L2 and the response namespace marker. See REVIEW.md for the scope conflict between L1 and HTTP(S) rewrites. |
...
[Chapter specification](namespaces/ipfs/SPEC.md) · [Deviations and open questions](namespaces/ipfs/DEVIATIONS.md)
...
| [Routing](namespaces/router/SPEC.md), [Handshake resolution](namespaces/handshake/SPEC.md#6-the-resolution-algorithm), [Handshake content pointers](namespaces/handshake/SPEC.md#10-content-pointers), [shared trust states](SPEC.md#4-trust-states) | Shared resolution rules | Namespace selection, chain proofs, DNSSEC anchored to the on-chain DS, pointer grammar and trust-state aggregation. Applied in this chapter at SPEC §3, §6, §7.2 and §10. |
...
[Chapter specification](namespaces/arweave/SPEC.md) · [Deviations and open questions](namespaces/arweave/DEVIATIONS.md)
...
| Identifier | Title | Used for |
|---|---|---|
| [Handshake §6](namespaces/handshake/SPEC.md#6-the-resolution-algorithm) | Handshake resolution | The chain resource, authoritative walk and DNSSEC validation of the name-to-Arweave pointer. This chapter uses that binding in SPEC §9.4. |
| [Experimental Part A](namespaces/experimental/SPEC.md#part-a--hip-5-_op-on-chain-resolution) | HIP-5 `_op` on-chain resolution | The contract route that returns an `arweave-ns` contenthash. This chapter starts at decoding that value (SPEC §5.3). |
| [Handshake §10](namespaces/handshake/SPEC.md#10-content-pointers) | Content pointers | The `ar=<txid>` grammar and precedence, implemented in `../../src/pointers.js`. Referenced by SPEC §5.2. |
| [Router §6](namespaces/router/SPEC.md#6-classification-of-bare-input), [shared trust states](SPEC.md#4-trust-states) | Namespace selection and trust states | Why an ar.io gateway name selects ICANN (SPEC §8), and how the trust model represents the result (SPEC §9.2; AR-U5). |
| [Handshake deviations](namespaces/handshake/DEVIATIONS.md), [Handshake §9.1](namespaces/handshake/SPEC.md#91-doh-fallback) | Shared cache and DoH limits | HS-1 tracks the fixed positive-cache lifetime; HS-8 describes resolver-trusted TLSA pins. The Handshake specification defines when DoH fallback is allowed. AR-3 and SPEC §9.4 refer to these limits. |
...
[Chapter specification](namespaces/ens/SPEC.md) · [Deviations and open questions](namespaces/ens/DEVIATIONS.md)
...
Paths refer to this chapter unless stated otherwise.
...
| [W3C DID Specification Registries](https://www.w3.org/TR/did-spec-registries/) | DID Specification Registries, W3C Group Note | §5.1 where a method name is looked up. Cited for what is refused: methods outside `plc`, `web`, `key`, `jwk` and `pkh` are refused before a network request. — `src/did-protocol.js` |
| [RFC 9498](https://www.rfc-editor.org/rfc/rfc9498) | The GNU Name System | §3 namespace precedence — resolve in the alternative namespace when its suffix matches, and do not continue into DNS on failure. The rule is in §9.10; `../../SPEC.md` adopts it and this chapter inherits it. — `../../src/router.js` |
...
| [did:key Method](https://w3c-ccg.github.io/did-key-spec/) | did:key method specification | §5.1, local expansion of a public-key identifier into a DID document. The implementation accepts base58btc keys with the codec and length checks listed there. — `src/did-local.js` |
| [did:jwk Method](https://github.com/quartzjer/did-jwk/blob/main/spec.md) | did:jwk method specification | §5.1, local expansion of a base64url JSON Web Key and the verification relationships selected by `use`. — `src/did-local.js` |
| [did:pkh Method](https://github.com/w3c-ccg/did-pkh/blob/main/did-pkh-method-draft.md) | did:pkh method draft | §5.1, CAIP-10 blockchain account identifiers and generated verification methods. — `src/did-local.js` |
...
| [W3C Controller Documents](https://www.w3.org/TR/controller-document/) | Controller Documents 1.0 (Multikey) | §5.1, the Multikey representation generated for supported `did:key` inputs. §10.5 also identifies the AT Protocol signing-key representation a future record verifier would consume. — `src/did-local.js`; AT-URI verification is not implemented. |

### Implementation dependencies

| Identifier | Title | Used for |
|---|---|---|
| [`@noble/curves`](https://github.com/paulmillr/noble-curves) | noble-curves — audited elliptic curve cryptography | §9.1 the secp256k1/BIP-340 implementation every receipt signature and verification goes through. Used by the receipt signing and verification modules. — `src/keys.js`, `src/nostr-event.js` |
| [`multiformats`](https://github.com/multiformats/js-multiformats) | Multiformat encodings | Base58btc decoding for local `did:key` expansion. — `src/did-local.js` |
...
[Chapter specification](namespaces/keys/SPEC.md) · [Deviations and open questions](namespaces/keys/DEVIATIONS.md)
