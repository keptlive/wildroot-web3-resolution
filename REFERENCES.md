...
| [`../apps/SPEC.md`](../apps/SPEC.md) §4 | Chapter 11 — Handshake applications and the `wss://` tunnel | Chapter 8 |
...
| [ar_tx.erl](https://github.com/ArweaveTeam/arweave/blob/master/apps/arweave/src/ar_tx.erl) | The node's own transaction module (repository source) | Chapter 4 |
...
| [arweave-js](https://github.com/ArweaveTeam/arweave-js) (`lib/deepHash.ts`, `lib/transaction.ts` `getSignatureData()`, `lib/crypto/node-driver.ts` `verify()`) | The Arweave JS client library (repository source, v1.15.7) | Chapter 4 |
...
| `browser src/hns/ws-proxy-policy.js` | Wildroot, the local-WebSocket policy and the revocation binding | Chapter 11 |
| [CAIP-2](https://chainagnostic.org/CAIPs/caip-2) / [CAIP-10](https://chainagnostic.org/CAIPs/caip-10) | Blockchain ID and Account ID specification | Chapter 7 |
...
| [did:jwk Method Specification](https://github.com/quartzjer/did-jwk/blob/main/spec.md) | did:jwk Method Specification | Chapter 7 |
| [did:key Method Specification](https://w3c-ccg.github.io/did-method-key/) | did:key Method Specification, W3C Credentials Community Group draft | Chapter 7 |
| [did:pkh Method Specification](https://github.com/w3c-ccg/did-pkh/blob/main/did-pkh-method-draft.md) | did:pkh Method Specification, W3C Credentials Community Group draft | Chapter 7 |
...
| [ENSIP-12](https://docs.ens.domains/ensip/12) | Avatar Text Records | Chapter 5 |
...
| [kubo `Datastore` configuration](https://github.com/ipfs/kubo/blob/master/docs/config.md#datastore) | kubo config: `StorageMax`, `StorageGCWatermark`, `GCPeriod` | Chapter 3 |
...
| [multicodec](https://github.com/multiformats/multicodec) | Multiformats — multicodec table | Chapter 5, Chapter 7 |
| [multicodec table.csv](https://github.com/multiformats/multicodec/blob/master/table.csv) | The multicodec code table | Chapter 4 |
| [`multiformats`](https://github.com/multiformats/js-multiformats) | js-multiformats | Chapter 7 |
...
| [Node `crypto`](https://nodejs.org/api/crypto.html) | Node.js `node:crypto` — `createPublicKey`, `ECDH.convertKey` | Chapter 7 |
...
| [RFC 1035](https://www.rfc-editor.org/rfc/rfc1035) §4.1.1 (header), §4.1.2 (question), §4.1.4 (message compression), §3.2.1 (TTL) | Domain Names — Implementation and Specification | Chapter 2 |
...
| [RFC 3986](https://www.rfc-editor.org/rfc/rfc3986) | Uniform Resource Identifier (URI): Generic Syntax | Part II, Chapter 1, Chapter 4, Chapter 6, Chapter 7, Chapter 9, Chapter 11 |
...
| [RFC 6066](https://www.rfc-editor.org/rfc/rfc6066) | TLS extensions: extension definitions | Chapter 1, Chapter 6, Chapter 11 |
...
| [RFC 7517](https://www.rfc-editor.org/rfc/rfc7517) | JSON Web Key (JWK) | Chapter 7 |
| [RFC 7518](https://www.rfc-editor.org/rfc/rfc7518) | JSON Web Algorithms (JWA) | Chapter 7 |
...
| [RFC 7748](https://www.rfc-editor.org/rfc/rfc7748) | Elliptic Curves for Security (Curve25519, Curve448) | Chapter 7 |
...
| [RFC 8017](https://www.rfc-editor.org/rfc/rfc8017) | PKCS #1 v2.2: RSA Cryptography Specifications | Chapter 4, Chapter 7 |
| [RFC 8032](https://www.rfc-editor.org/rfc/rfc8032) | Edwards-Curve Digital Signature Algorithm (EdDSA) | Chapter 7, Chapter 8, Chapter 9 |
| [RFC 8037](https://www.rfc-editor.org/rfc/rfc8037) | CFRG ECDH and Signatures in JOSE | Chapter 7 |
...
| [RFC 8446](https://www.rfc-editor.org/rfc/rfc8446) | The transport layer security (TLS) protocol version 1.3 | Chapter 1, Chapter 2, Chapter 6, Chapter 8, Chapter 9, Chapter 11 |
...
| [RFC 9111](https://www.rfc-editor.org/rfc/rfc9111) | HTTP caching | Chapter 1 |
...
| [RFC 9848](https://www.rfc-editor.org/rfc/rfc9848) | TLS encrypted client hello | Chapter 1, Chapter 2, Chapter 11 |
...
| [webRequest.onBeforeRequest](https://www.electronjs.org/docs/latest/api/web-request#webrequestonbeforerequestfilter-listener) | Electron API — `webRequest.onBeforeRequest` | Chapter 8 |
| [WHATWG Fetch](https://fetch.spec.whatwg.org/) | Fetch Living Standard | Part II, Chapter 1, Chapter 7, Chapter 11 |
...
| [IANA URI Schemes registry](https://www.iana.org/assignments/uri-schemes/) | Uniform Resource Identifier (URI) Schemes | Checked, not assumed: the "IANA" column of SPEC §4.2 is this registry's own status field. Of the 30 schemes in the table, 2 are Permanent, 11 Provisional, 17 unregistered. SPEC §4.2, RT-6 |
...
| [W3C DID Core](https://www.w3.org/TR/did-core/) | Decentralized Identifiers (DIDs) v1.0 | The `did` row's object, and the method syntax the trust step reads: `did:plc` and `did:web` retrieve a document over HTTPS and have its id checked (the PLC operation log that would prove it is not audited), while `did:key`, `did:jwk` and `did:pkh` derive one from the identifier itself with no lookup — which proves nobody's possession of a key. Every arm is `unverified` and says the panel holds no recorded result for the page. SPEC §4.2, §10.2 · `src/trust-path.js` |
...
| Identifier | Title | Used for |
|---|---|---|
| [RFC 1034](https://www.rfc-editor.org/rfc/rfc1034) | Domain names — concepts and facilities | §6.5c, the rule that tells a **referral** from a **NODATA** (§4.3.2: NS in AUTHORITY with no SOA is a referral), which is what makes the registry-TLD walk possible; §6.5f, §3.6.2 — a CNAME stands alone, so a validated CNAME needs no separate NSEC proving the `A` absent. §3.6.2 is also the alias handling of §6.5e rule 4 — the query is restarted at the canonical name, and a loop is signalled as an error — which this implementation performs only inside the zone already anchored (HS-17) and only to a bounded depth (HS-18). §HS-15 cites it for the NS set. `../../src/resolver.js` (`referralIn`, `_validatedTxtAnswer`, the CNAME branch of `_fromZone`) |
...
| [RFC 2181](https://www.rfc-editor.org/rfc/rfc2181) | Clarifications to the DNS specification | §5.2 TTL rules, cited by the caching deviation of §6.8. **§10.1** is the precise statement behind §6.5e rule 2 and the one-alias bound: only one canonical name per alias, and for any label exactly one of — a CNAME with its DNSSEC records, non-CNAME records, nothing, or no such name. So a reply carrying `TXT` and `CNAME` at one owner, or two `CNAME`s at one owner, is refused. `../../src/resolver.js:863`, `:869` (`_validatedTxtAnswer`) — see HS-1, HS-18 |
...
| [RFC 1928](https://www.rfc-editor.org/rfc/rfc1928) | SOCKS protocol version 5 | §6.11: the `dial` seam. A CONNECT request (`no authentication` method only) to a device-local SOCKS port carries the authoritative TCP query of §6.5 while an anonymizing proxy is on, so the chain proof and the DNSSEC validation are kept rather than traded away. A dotted quad goes out as ATYP `0x01`; anything else as ATYP `0x03`, a domain name, resolved by the proxy and never locally. §8.1: in Private mode the same dialler carries the A-record site's TLS socket, **by address** (ATYP `0x01`), so the name is absent from the CONNECT request while the DANE pin is checked on that handshake — which is a disclosure removed from the *proxy*, not from the wire: the ClientHello that follows still carries the name (RFC 8446 §4.2.9 / RFC 6066 §3, below). Which proxy, and what it is worth, is Chapter 8. `../../src/socks-dial.js:104` (`socksDialer`; a dial settles once and its timer is unref'd, and the optional `{ signal, pauseOnConnect }` is for callers outside this chapter), `../../src/dns-query.js` (`query`'s `dial` option), `../../src/dane-connect.js` (`connectDane`'s and `connectPlain`'s `dial`) |
...
| [RFC 4034](https://www.rfc-editor.org/rfc/rfc4034) | Resource records for the DNS security extensions | §6.7: DNSKEY (§2), RRSIG (§3) including the **Labels** rule of §3.1.3, DS (§5), NSEC (§4), canonical RR form (§6.2) and canonical NAME ordering (§6.1); §2.1.1 is the Zone Key flag; §3.1.5 the validity window (HS-10). **§6.2 is also what a `CNAME` RRset is validated *as*** on the pointer path of §6.5e: the signing input is the target name lowercased in wire form, built by `wireName()` rather than taken from the parsed reply. `../../src/dnssec.js`, `../../src/nsec.js`, `../../src/resolver.js:822` (`_validateTxtRRset`) |
| [RFC 4035](https://www.rfc-editor.org/rfc/rfc4035) | Protocol modifications for the DNS security extensions | The validator's whole job in §6.5–§6.7: §5.2 DS→DNSKEY authentication including that a missing DS must be proven missing, §5.3 RRSIG validation, §5.3.1 CNAME handling (HS-9), §5.3.2 wildcard signature reconstruction, §5.3.4 the wildcard-answer proof, §5.4 authenticated denial, §5.5 "unvalidatable ⇒ Bogus", §3.1.3.4 wildcard NODATA. **§5.3 and §5.4 together are the one rule of §6.5e**: every `TXT` or `CNAME` RRset at a pointer owner is validated before it is read as application syntax (`_validateTxtRRset`, `../../src/resolver.js:816`), and an owner with neither must have its `TXT` denied by a validated NSEC/NSEC3 (`_validatedTxtAnswer`, `:849`, `:886`). `../../src/dnssec.js`, `../../src/denial.js`, `../../src/resolver.js` |
...
| [RFC 8446](https://www.rfc-editor.org/rfc/rfc8446) | The transport layer security (TLS) protocol version 1.3 | §8, §8.1: the transport an `hns://` fetch runs over, terminated by Node's TLS stack; the peer certificate it yields is what the pin is checked against. `connectDane` sets `servername` to the Handshake name, turns PKIX verification off, and layers the handshake over a direct socket or the SOCKS tunnel alike. **§4.2.9** is the `server_name` extension that setting produces: sent in the clear before any key is agreed, on both routes, which is why §8.1 and §11.6 state that the Tor exit sees the name (HS-3). `../../src/dane-connect.js:38`; the request written over it is the composition layer's (DEVIATIONS §4) |
| [RFC 6066](https://www.rfc-editor.org/rfc/rfc6066) | TLS extensions: extension definitions | §8.1, §11.6: **§3**, Server Name Indication — the extension RFC 8446 §4.2.9 carries forward, and the reason a Handshake name reaches the site and everything between. It is what the site needs to select a certificate the pin can be checked against, so it cannot simply be omitted; removing it needs ECH (RFC 9848), which this transport cannot use. `../../src/dane-connect.js:38` (`connectDane`'s `servername`) — HS-3 |
...
| [RFC 9111](https://www.rfc-editor.org/rfc/rfc9111) | HTTP caching | §6.8: the `no-cache` **request** directive (§5.2.1.4, obsoleting RFC 7234 §5.2.1.4) sent with a DoH query for a name that was just invalidated, so the HTTP client under the resolver revalidates instead of reusing a stored response. It is a directive to that client's own store: it is not a DNS answer, it changes no trust decision, and it does not reach the upstream recursive resolver's cache. `../../src/doh.js:205`, marked by `_fresh()` at `:91` — HS-1 |
| [WHATWG Fetch](https://fetch.spec.whatwg.org/#concept-request-cache-mode) | Fetch Standard — request cache mode | §6.8: `cache: 'reload'`, the request cache mode paired with the directive above — fetch without consulting the HTTP cache, and update it with the response. Set only for a recently forgotten name and the names under it. `../../src/doh.js:204` |
...
| Identifier | Title | Used for |
|---|---|---|
| [RFC 3986](https://www.rfc-editor.org/rfc/rfc3986) | Uniform resource identifier (URI): generic syntax | §10.1: **§6.2.1**, simple character-for-character comparison, is how a content pointer's trailing path is compared when the record at the name and the DNSLink record are merged; the syntax-based normalisations of **§6.2.2** (case, percent-encoding, path segments) are deliberately not applied, because in a content-addressed path the spelling selects the link. `../../src/pointers.js:328` (`mergePointers`) — HS-19 |
...
| [`SPEC.md` §4.2](SPEC.md) | The Fast / Private switch | §4.1, §8.1, §9.3, §10.2, §11.6: the one control whose policy table (`policyFor`) this chapter's consumers read — `strictOblivious` for the DoH resolver, the route for the site socket and the pointer decisions — and the one builder of every mode-caused page, `privateRefusal('lookup' \| 'site' \| 'ipfs' \| 'p2p')` — whose `'p2p'` text states that a stated origin does **not** enable those protocols in Private mode, because the whole-archive remedy of §10.2 is IPFS's alone. The disclosure the control carries is `DISCLOSURE`, in full: it holds for the **default** DNS setting, and says that a private lookup fails rather than falling back **while a cached answer may still work** — configuration is evidence about the next query, not about what a page load used (§9.3). `../../src/delivery-mode.js`, `../../tests/delivery-mode.test.js` |
...
| [RFC 9498](https://www.rfc-editor.org/rfc/rfc9498) §9.10 | The GNU Name System | SPEC §3 — resolve in the alternative namespace when its identifier matches, and do **not** continue into another namespace on failure. Inherited from the spine as the rule for both schemes. `../../src/router.js` |
...
| Identifier | Title | Used for |
|---|---|---|
| [ipfs/kubo](https://github.com/ipfs/kubo) | kubo (go-ipfs) | SPEC §5, §7.3, §12.4, §12.5 — the one node every retrieval goes through, and therefore the implementation whose IPNS record checking, block verification and DAG defaults this chapter relies on. Version 0.43 defaults are what `src/cid.js` reproduces and `tests/cid.test.js` pins; its privacy configuration is the subject of SPEC §12.4 |
| [kubo `Datastore` configuration](https://github.com/ipfs/kubo/blob/master/docs/config.md#datastore) | kubo config: `StorageMax`, `StorageGCWatermark`, `GCPeriod` | SPEC §8.3 — the garbage collector a warmed archive is exposed to, because a stated-origin import is deliberately unpinned (SPEC §12.2). It removes every unpinned block, runs at most once per `GCPeriod` and only above the watermark, and gives no notification — which is why what the warmer remembers expires and is re-confirmed rather than held (IP-12). `src/block-presence.js`, `src/origin-warm.js` |
...
| [ar_tx.erl](https://github.com/ArweaveTeam/arweave/blob/master/apps/arweave/src/ar_tx.erl) | The node's own transaction module (repository source) | The authority the JS library follows, and the source of the two details it does not state plainly: the optional `denomination`, prepended to the format-2 payload when the header carries one, and the field limits a transaction cannot exceed (a 32-byte target, a 32- or 48-byte anchor, the tag budget). `src/ar-tx.js` reads all of them strictly — SPEC §9.1.1. |
...
| [RFC 1928](https://www.rfc-editor.org/rfc/rfc1928) | SOCKS Protocol Version 5 | **§8.5**: the Private route. The Tor-dialling class is a SOCKS5 client of the device-local Tor, offering only the "no authentication" method (§3) and sending the relay's hostname as address type `0x03` `DOMAINNAME` (§4), so the name is resolved inside Tor and the operating system's resolver is never asked — the test asserts the address type on the wire. RFC 1929's username/password method is not used, so **no stream isolation is requested**: which circuit each relay dial is assigned to is Tor's own decision, is not observed here, and MUST NOT be described as though it were known (Chapter 8, TO-3). `src/tor-websocket.js` via `socksDialer`, `../../src/socks-dial.js` — shared with the Handshake resolver's authoritative hop, the WebSocket tunnel of Chapter 11 and `gemini://` of Chapter 9 |
...
| [`namespaces/apps/SPEC.md`](namespaces/apps/SPEC.md) §4 | Chapter 11 — Handshake applications and the `wss://` tunnel | §3.1, the owner of the `websocketPolicy` this chapter's single `onBeforeRequest` listener dispatches `ws:`/`wss:` to. What the policy decides is specified there; what is specified here is that it is consulted before anything else, that no WebSocket enters the ad blocker or the HTTP namespace rewrite, and that a policy fault cancels (R18). §7.5 also names the tunnel as one of the five main-process paths that dial this controller's SOCKS port for themselves. |
...
| Identifier | Title | Used for |
|---|---|---|
| [RFC 3986](https://www.rfc-editor.org/rfc/rfc3986) | Uniform Resource Identifier (URI): Generic Syntax | The generic grammar every identifier here is written in. §3.1 (scheme names are case-insensitive) is why the router lowercases before dispatch; §3.2.2 (host) is what a hypercore key or an info-hash *is* in the URL form; §4.2 (relative references) is what a Gemini redirect target may be; §3.3 (opaque path, no authority) is why `magnet:` is registered non-standard and is never an origin; §3.4 (query) is the component a magnet's whole meaning lives in, read through `URLSearchParams` — every `xt` and every `xs`, not the first of each (SPEC §K.7.4). SPEC §K.3.4, §K.6.1, §K.7.1, §K.7.4 — `../../src/router.js`, `src/magnet-protocol.js`, `src/gemini-protocol.js`, `src/torrent-address.js` |
...
| [BEP 9](https://www.bittorrent.org/beps/bep_0009.html) | Extension for Peers to Send Metadata Files | The **magnet URI format** (`xt=urn:btih:`, `dn=`, `tr=`) and the `ut_metadata` exchange that lets a client start from a magnet with no `.torrent`. BEP 9 permits the info-hash as 40 hex **or** 32 base32 characters; only hex is accepted and the base32 form is refused by name (`DEVIATIONS.md` KY-2). BEP 9 says what a magnet *means*, not what a browser does when one is clicked: here an `xt=urn:btih:` magnet becomes a confirmation page and no peer is dialled until the user agrees (SPEC §K.7.5). The metadata exchange is **via the engine**. SPEC §K.7.1, §K.7.4, §K.7.5 — `src/magnet-protocol.js`, `src/torrent-input.js` |
...
| [BEP 46](https://www.bittorrent.org/beps/bep_0046.html) | Updating Torrents Via DHT Mutable Items | The **64-hex `btpk`**: an Ed25519 public key whose DHT mutable item names a changing info-hash, and the `xs=urn:btpk:` magnet parameter. The one *key*-addressed form in the BitTorrent namespace, and the reason key shape is the whole dispatch. Opening a BEP-46 address dials peers exactly as adding an infohash does, so the `xs=` magnet takes the same consent path as the `xt=` one and is distinguished only by which branch of the confirmation page it names (SPEC §K.7.5). An `xs` whose key is not 64 hex is refused as though the magnet carried no `xs` (`DEVIATIONS.md` KY-12). The signature check is **via the engine** (`bt-fetch`). SPEC §K.7.1, §K.7.2, §K.7.4, §K.7.5, §K.7.8, §K.9 — `src/magnet-protocol.js` (`PUBLIC_KEY_MATCH`), `src/torrent-address.js`, `src/torrent-input.js` |
...
| [BEP 53](https://www.bittorrent.org/beps/bep_0053.html) | Magnet URI extension — Select specific file indices | The `so=` parameter. **Not implemented** — it is ignored, along with every magnet parameter except `xt`, `xs` and `dn`, none of which survives the 308 into the confirmation page. Its job (choose which files of a torrent to fetch) is done by the URL path instead. SPEC §K.7.4, §K.7.7; `DEVIATIONS.md` KY-10 |
...
| `../../src/delivery-mode.js` | The one switch — Settings › Content delivery › Mode | `policyFor()`, whose `p2pDiscovery` row is what §K.3.6 enforces; `privateRefusal('p2p', { label })`, the words the gate answers with; its `host` form, which a named peer-to-peer site gets and which says that a stated origin does **not** enable this protocol in Private mode (`:184`) — the origin path is Chapter 3's IPFS CAR fetch and has no counterpart here; `SWITCH_HINT`, its last sentence. `../../DIVERGENCE.md` row 19 is the inventory entry. SPEC §K.3.6, §K.8 — `src/gate.js`, `../ipfs/src/origin-warm.js` |
